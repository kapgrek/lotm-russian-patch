...
class TranslateBatch8DialogueQuests
{
    ...
    static readonly Dictionary<string, string> CanonExact = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        { "Merman Scales", "Чешуя тритона" },
        { "Siren Stone", "Камень сирены" },
        { "Siren Stones", "Камни сирен" },
    ...
    static void Main(string[] args)
    {
        ...
        int targetCount = 2200;
        if (args.Length > 0) int.TryParse(args[0], out targetCount);

        Console.WriteLine("==========================================================================");
        Console.WriteLine(" LOTM - Пакет №8: Диалоги персонажей, уличные сцены и сюжетные квесты      ");
        Console.WriteLine("==========================================================================");
        ...
        var pSideTask = new List<Tuple<string, string>>();
        var pTalk = new List<Tuple<string, string>>();
        var pMainTask = new List<Tuple<string, string>>();
        var pTingenTalk = new List<Tuple<string, string>>();
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        // Игнорируем технические комментарии разработчиков
                        ...
                        bool isSide = tags.Contains("sidetask") || tags.Contains("itemtask");
                        bool isTalk = tags.Contains("talk");
                        bool isMain = tags.Contains("maintask") || tags.Contains("main");
                        bool isTingen = tags.Contains("tingentalk") || tags.Contains("tingen");
                        bool isOther = tags.Contains("othertalk") || tags.Contains("asidetalk") || tags.Contains("talkother");

                        if (isSide)
                        {
                            seen.Add(cnKey); seen.Add(enVal);
                            pSideTask.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isTalk)
                        {
                            seen.Add(cnKey); seen.Add(enVal);
                            pTalk.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isMain)
                        {
                            seen.Add(cnKey); seen.Add(enVal);
                            pMainTask.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isTingen)
                        {
                            seen.Add(cnKey); seen.Add(enVal);
                            pTingenTalk.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isOther)
                        {
                            seen.Add(cnKey); seen.Add(enVal);
        ...
        Console.WriteLine(string.Format("Найдено кандидатов:\n  - SideTask (побочные квесты): {0}\n  - Talk (мысли и диалоги сцен): {1}\n  - MainTask (основной сюжет): {2}\n  - TingenTalk (Тинген): {3}\n  - OtherTalk (уличные реплики): {4}",
            pSideTask.Count, pTalk.Count, pMainTask.Count, pTingenTalk.Count, pOtherTalk.Count));

        // Формирование набора для перевода (Все SideTask + Все Talk + до 800 MainTask + до 600 TingenTalk)
        var candidates = new List<Tuple<string, string>>();

        // 1. Все оставшиеся побочные задания
        foreach (var c in pSideTask)
        {
            candidates.Add(c);
        }
        Console.WriteLine(string.Format("Добавлено SideTask: {0}", candidates.Count));

        // 2. Все оставшиеся мысли/диалоги сцен talk
        int before = candidates.Count;
        foreach (var c in pTalk)
        {
            candidates.Add(c);
        }
        Console.WriteLine(string.Format("Добавлено Talk: {0}", candidates.Count - before));

        // 3. До 800 ключевых сюжетных строк MainTask
        before = candidates.Count;
        int mainAdded = 0;
        foreach (var c in pMainTask)
        {
            if (mainAdded >= 800) break;
            candidates.Add(c);
            mainAdded++;
        }
        Console.WriteLine(string.Format("Добавлено MainTask: {0}", candidates.Count - before));

        // 4. До 600 диалогов Тингена
        before = candidates.Count;
        int tingenAdded = 0;
        foreach (var c in pTingenTalk)
        {
            if (tingenAdded >= 600) break;
            candidates.Add(c);
            tingenAdded++;
        }
        Console.WriteLine(string.Format("Добавлено TingenTalk: {0}", candidates.Count - before));

        // 5. Если лимит еще не достигнут, добираем из OtherTalk
        if (candidates.Count < targetCount)
        {
            before = candidates.Count;
            foreach (var c in pOtherTalk)
            {
                if (candidates.Count >= targetCount) break;
                candidates.Add(c);
            }
            Console.WriteLine(string.Format("Добавлено OtherTalk: {0}", candidates.Count - before));
        }

        Console.WriteLine(string.Format("Итого отобрано в Пакет №8: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №8 успешно интегрирован!");
    }
