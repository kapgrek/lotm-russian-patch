...
class InspectBatch24Details
{
    ...
    static void Main()
    {
        ...
        Console.WriteLine("--- High priority (first 30): ---");
        for (int i = 0; i < Math.Min(30, list.Count); i++)
        {
            Console.WriteLine(string.Format("{0}: EN: {1} | CN: {2}", i + 1, list[i].Item3, list[i].Item2));
        }

        Console.WriteLine("\n--- Middle priority (around 4000): ---");
        for (int i = 4000; i < Math.Min(4025, list.Count); i++)
        {
            Console.WriteLine(string.Format("{0}: EN: {1} | CN: {2}", i + 1, list[i].Item3, list[i].Item2));
        }

        Console.WriteLine("\n--- Low priority (last 30): ---");
        for (int i = Math.Max(0, list.Count - 30); i < list.Count; i++)
        {
            Console.WriteLine(string.Format("{0}: EN: {1} | CN: {2}", i + 1, list[i].Item3, list[i].Item2));
        }
    }
