...
class TranslateBatch4SideQuests
{
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("==========================================================");
        Console.WriteLine("   LOTM - Пакет №4: Побочные квесты Тингена и Бэкланда    ");
        Console.WriteLine("==========================================================");
        ...
        // 3. Отбор кандидатов для Пакета №4
        var p1TingenTalk = new List<Tuple<string, string>>();    // Диалоги Тингена (tingentalk, tingen)
        var p2Quests = new List<Tuple<string, string>>();        // Квесты и задачи (quest, task)
        var p3AsideTalk = new List<Tuple<string, string>>();      // Уличные реплики и сценки (asidetalk, othertalk, talkother)
        var p4OldTalk = new List<Tuple<string, string>>();        // Диалоги жителей и NPC (oldtalk, talk)
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        // Игнорируем внутренние технические ID
                        ...
                        bool isTingenTalk = tags.Contains("tingentalk") || tags.Contains("tingen");
                        bool isQuest = tags.Contains("quest") || tags.Contains("task") || tags.Contains("questtext") || cnKey.Contains("任务");
                        bool isAsideTalk = tags.Contains("asidetalk") || tags.Contains("othertalk") || tags.Contains("talkother");
                        bool isOldTalk = tags.Contains("oldtalk") || tags.Contains("talk");

                        if (isTingenTalk)
                        {
                            seenCn.Add(cnKey);
                            p1TingenTalk.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isQuest)
                        {
                            seenCn.Add(cnKey);
                            p2Quests.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isAsideTalk)
                        {
                            seenCn.Add(cnKey);
                            p3AsideTalk.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isOldTalk)
                        {
                            seenCn.Add(cnKey);
                            p4OldTalk.Add(Tuple.Create(cnKey, enVal));
                        }
                    }
                }
            }
        ...
        Console.WriteLine(string.Format("Найдено кандидатов для Пакета №4:\n  - P1 (Диалоги Тингена): {0}\n  - P2 (Квесты и задачи): {1}\n  - P3 (Уличные сценки и реплики): {2}\n  - P4 (Диалоги NPC): {3}",
            p1TingenTalk.Count, p2Quests.Count, p3AsideTalk.Count, p4OldTalk.Count));

        // Формирование набора для перевода
        var candidates = new List<Tuple<string, string>>();
        foreach (var c in p1TingenTalk)
        {
            if (candidates.Count >= targetCount) break;
            candidates.Add(c);
        }
        foreach (var c in p2Quests)
        {
            if (candidates.Count >= targetCount) break;
            candidates.Add(c);
        }
        foreach (var c in p3AsideTalk)
        {
            if (candidates.Count >= targetCount) break;
            candidates.Add(c);
        }
        foreach (var c in p4OldTalk)
        {
            if (candidates.Count >= targetCount) break;
            candidates.Add(c);
        }

        Console.WriteLine(string.Format("Итого отобрано в Пакет №4: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №4 успешно интегрирован!");
    }
    ...
    static string ApplyCanonGlossary(string text)
    {
        ...
        // Канонизация и устранение машинных ошибок в диалогах и квестах
