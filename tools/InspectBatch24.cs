...
class InspectBatch24
{
    ...
    static void Main()
    {
        ...
        var technicalList = new List<Tuple<string, string>>();
        var seen = new HashSet<string>(StringComparer.Ordinal);
        int totalGeminiLines = 0;
        int totalUntranslatedInGemini = 0;
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        bool translated = existingRu.Contains(cnKey) || existingRu.Contains(enVal);
                        if (!translated)
                        {
                            totalUntranslatedInGemini++;
                        }
                        ...
                        {
                            technicalList.Add(Tuple.Create(cnKey, enVal));
                            continue;
                        }
        ...
        Console.WriteLine(string.Format("Total Gemini source lines: {0}", totalGeminiLines));
        Console.WriteLine(string.Format("Total existing entries in Ru: {0}", existingRu.Count));
        Console.WriteLine(string.Format("Total untranslated in Gemini: {0}", totalUntranslatedInGemini));
        Console.WriteLine(string.Format("Filtered candidate pool for Batch 24: {0}", list.Count));
        Console.WriteLine(string.Format("Technical/debug excluded: {0}", technicalList.Count));

        Console.WriteLine("\nSamples across Batch 24:");
        int step = list.Count > 10 ? list.Count / 10 : 1;
        ...
        Console.WriteLine("\nSamples of Technical/Debug excluded:");
        int tStep = technicalList.Count > 5 ? technicalList.Count / 5 : 1;
        for (int i = 0; i < technicalList.Count; i += tStep)
        {
            Console.WriteLine(string.Format("  [T-{0}] EN: {1} | CN: {2}", i, technicalList[i].Item2, technicalList[i].Item1));
        }
    }
}
