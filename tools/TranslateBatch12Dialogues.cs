...
class TranslateBatch12Dialogues
{
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("==========================================================================");
        Console.WriteLine(" LOTM - Пакет №12: Карточные партии, сплетни горожан и поручения (55%+ игры)");
        Console.WriteLine("==========================================================================");
        ...
        // 1. OldTalk (1200 строк)
        int oldLimit = Math.Min(1200, pOldTalk.Count);
        ...
        for (int i = 0; i < gossipLimit; i++) candidates.Add(pGossip[i]);
        Console.WriteLine(string.Format("Добавлено Gossip: {0}", gossipLimit));

        // 3. TalkOther (500 строк)
        int talkOtherLimit = Math.Min(500, pTalkOther.Count);
        ...
        if (candidates.Count < targetCount)
        {
            ...
            for (int i = gossipLimit; i < pGossip.Count && candidates.Count < targetCount; i++) candidates.Add(pGossip[i]);
            ...
            for (int i = 0; i < pAsideTalk.Count && candidates.Count < targetCount; i++) candidates.Add(pAsideTalk[i]);
            Console.WriteLine(string.Format("Дополнительно добрано до целевого объема: {0}", candidates.Count - before));
        }

        Console.WriteLine(string.Format("Итого отобрано в Пакет №12: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №12 успешно интегрирован!");
    }
