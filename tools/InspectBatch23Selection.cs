...
class InspectBatch23Selection
{
    ...
    static void Main()
    {
        ...
        Console.WriteLine(string.Format("Filtered candidate pool for Batch 23: {0}", list.Count));

        Console.WriteLine("\nSamples across Batch 23 (target 8800):");
        int[] indices = new int[] { 0, 10, 50, 100, 500, 1000, 2500, 4500, 6500, 8500, 8799 };
        foreach (int idx in indices)
        {
            if (idx < list.Count)
            {
                Console.WriteLine(string.Format("  [{0}] (Score {1}) EN: {2} | CN: {3}", idx, list[idx].Item1, list[idx].Item3, list[idx].Item2));
            }
        }
    }
}
