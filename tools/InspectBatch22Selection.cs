...
class InspectBatch22Selection
{
    ...
    static int GetStringPriorityScore(string enVal, string cnKey)
    {
        ...
        // 6. Любой связный английский текст с пробелами (фразы от 2 слов)
        if (enVal.Contains(" "))
        {
            score += 20;
        }
    ...
    static void Main()
    {
        ...
        Console.WriteLine(string.Format("Filtered candidate pool for Batch 22: {0}", list.Count));
        Console.WriteLine("\nTop 20 candidates for Batch 22:");
        ...
        int mid = Math.Min(4000, list.Count - 1);
        if (mid >= 0)
        {
            Console.WriteLine(string.Format("\nCandidate #{0}:", mid));
            Console.WriteLine(string.Format("  [{0}] (Score {1}) EN: {2} | CN: {3}", mid, list[mid].Item1, list[mid].Item3, list[mid].Item2));
        }

        int target = Math.Min(8499, list.Count - 1);
        if (target >= 0)
        {
            Console.WriteLine(string.Format("\nCandidate #{0}:", target));
            Console.WriteLine(string.Format("  [{0}] (Score {1}) EN: {2} | CN: {3}", target, list[target].Item1, list[target].Item3, list[target].Item2));
        }
    }
}
