...
class InspectBatch14Candidates
{
    ...
    static void Main()
    {
        ...
        var itemGift = new List<Tuple<string, string>>();
        var itemOutlook = new List<Tuple<string, string>>();
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        if (lsiTagMap.TryGetValue(sk, out tags))
                        {
                            ...
                            else if (tags.Contains("itemnormal")) itemNormal.Add(Tuple.Create(cnKey, enVal));
                            else if (tags.Contains("itemgift")) itemGift.Add(Tuple.Create(cnKey, enVal));
                            else if (tags.Contains("itemoutlook")) itemOutlook.Add(Tuple.Create(cnKey, enVal));
                            else if (tags.Contains("gossip")) gossip.Add(Tuple.Create(cnKey, enVal));
                            else if (tags.Contains("monsterskill")) monsterSkill.Add(Tuple.Create(cnKey, enVal));
        ...
        Console.WriteLine("\n=== Tagged Candidate Breakdown for Batch 14 ===");
        Console.WriteLine("  monsterskill: " + monsterSkill.Count);
        Console.WriteLine("  itemnormal: " + itemNormal.Count);
        Console.WriteLine("  talkother: " + talkOther.Count);
        Console.WriteLine("  itemgift: " + itemGift.Count);
        ...
        Console.WriteLine("  itemoutlook: " + itemOutlook.Count);
        ...
        for (int i = 0; i < Math.Min(30, sortedTags.Count); i++)
        {
