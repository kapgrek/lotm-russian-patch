...
class TranslateBatch7MainQuests
{
    ...
    static readonly Dictionary<string, string> CanonExact = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // Локации, районы Бэкланда и организации
        { "Backlund", "Бэкланд" },
        { "Tingen", "Тинген" },
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("==========================================================================");
        Console.WriteLine(" LOTM - Пакет №7: Основные сюжетные квесты (maintask) и городские сплетни ");
        Console.WriteLine("==========================================================================");
        ...
        var p1MainTask = new List<Tuple<string, string>>();
        var p2SideTask = new List<Tuple<string, string>>();
        var p3Gossip = new List<Tuple<string, string>>();
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        if (existingRu.ContainsKey(cnKey) || existingRu.ContainsKey(enVal) || seenCn.Contains(cnKey) || seenCn.Contains(enVal))
                            continue;
                        ...
                        if (isMain)
                        {
                            seenCn.Add(cnKey); seenCn.Add(enVal);
                            p1MainTask.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isSide)
                        {
                            seenCn.Add(cnKey); seenCn.Add(enVal);
                            p2SideTask.Add(Tuple.Create(cnKey, enVal));
                        }
                        else if (isGossip)
                        {
                            seenCn.Add(cnKey); seenCn.Add(enVal);
                            p3Gossip.Add(Tuple.Create(cnKey, enVal));
                        }
                    }
                }
            }
        ...
        Console.WriteLine(string.Format("Найдено кандидатов для Пакета №7:\n  - P1 (Основные сюжетные задания maintask/main): {0}\n  - P2 (Побочные задания sidetask/itemtask): {1}\n  - P3 (Городские сплетни и реплики жителей gossip): {2}",
            p1MainTask.Count, p2SideTask.Count, p3Gossip.Count));

        // Формирование набора для перевода (1200 maintask + 400 sidetask + 400 gossip)
        var candidates = new List<Tuple<string, string>>();
        int mainLimit = 1200;
        int sideLimit = 400;
        int gossipLimit = 400;

        foreach (var c in p1MainTask)
        {
            if (candidates.Count >= mainLimit) break;
            candidates.Add(c);
        }
        int curCount = candidates.Count;
        foreach (var c in p2SideTask)
        {
            if (candidates.Count >= curCount + sideLimit) break;
            candidates.Add(c);
        }
        curCount = candidates.Count;
        foreach (var c in p3Gossip)
        {
            if (candidates.Count >= curCount + gossipLimit) break;
            candidates.Add(c);
        }

        // Если лимит еще не достигнут (например, меньше targetCount), добираем из оставшихся maintask
        ...
        Console.WriteLine(string.Format("Итого отобрано в Пакет №7: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №7 успешно интегрирован!");
    }
