...
class InspectBatch11Candidates
{
    ...
    static void Main()
    {
        ...
        var talkOtherCandidates = new List<Tuple<string, string>>();
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        if (lsiTagMap.TryGetValue(sk, out tags))
                        {
                            ...
                            if (isOld) oldTalkCandidates.Add(Tuple.Create(cnKey, enVal));
                            if (isTalkOther) talkOtherCandidates.Add(Tuple.Create(cnKey, enVal));
                            if (isTingen) tingenCandidates.Add(Tuple.Create(cnKey, enVal));
                            if (isOther) otherTalkCandidates.Add(Tuple.Create(cnKey, enVal));
                            if (isAside) asideTalkCandidates.Add(Tuple.Create(cnKey, enVal));
                            if (isGossip) gossipCandidates.Add(Tuple.Create(cnKey, enVal));
        ...
        Console.WriteLine("\nCandidate groups for Batch 11:");
        Console.WriteLine("  oldtalk: " + oldTalkCandidates.Count);
        Console.WriteLine("  talkother: " + talkOtherCandidates.Count);
        Console.WriteLine("  tingentalk / tingen: " + tingenCandidates.Count);
        Console.WriteLine("  othertalk: " + otherTalkCandidates.Count);
        Console.WriteLine("  asidetalk: " + asideTalkCandidates.Count);
        Console.WriteLine("  gossip: " + gossipCandidates.Count);

        Console.WriteLine("\nSample talkother:");
        for (int i = 0; i < Math.Min(5, talkOtherCandidates.Count); i++)
            Console.WriteLine("  EN: " + talkOtherCandidates[i].Item2);

        Console.WriteLine("\nSample oldtalk:");
        for (int i = 0; i < Math.Min(5, oldTalkCandidates.Count); i++)
            Console.WriteLine("  EN: " + oldTalkCandidates[i].Item2);

        Console.WriteLine("\nSample tingentalk:");
        for (int i = 0; i < Math.Min(5, tingenCandidates.Count); i++)
            Console.WriteLine("  EN: " + tingenCandidates[i].Item2);

        Console.WriteLine("\nSample othertalk:");
        for (int i = 0; i < Math.Min(5, otherTalkCandidates.Count); i++)
            Console.WriteLine("  EN: " + otherTalkCandidates[i].Item2);

        Console.WriteLine("\nSample asidetalk:");
        for (int i = 0; i < Math.Min(5, asideTalkCandidates.Count); i++)
            Console.WriteLine("  EN: " + asideTalkCandidates[i].Item2);
    }
}
