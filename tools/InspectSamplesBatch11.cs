...
class InspectSamples
{
    ...
    static void Main()
    {
        ...
        var sampleTalkOther = new List<string>();
        var sampleOldTalk = new List<string>();
        var sampleTingen = new List<string>();
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        {
                            if (tags.Contains("talkother") && sampleTalkOther.Count < 8) sampleTalkOther.Add(enVal);
                            if (tags.Contains("oldtalk") && sampleOldTalk.Count < 8) sampleOldTalk.Add(enVal);
                            if ((tags.Contains("tingentalk") || tags.Contains("tingen")) && sampleTingen.Count < 8) sampleTingen.Add(enVal);
                        }
                    }
                }
            }
        ...
        Console.WriteLine("--- TALKOTHER ---");
        foreach (var s in sampleTalkOther) Console.WriteLine("  " + s);
        Console.WriteLine("--- OLDTALK ---");
        foreach (var s in sampleOldTalk) Console.WriteLine("  " + s);
        Console.WriteLine("--- TINGEN ---");
        foreach (var s in sampleTingen) Console.WriteLine("  " + s);
    }
}
