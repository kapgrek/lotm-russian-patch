...
class InspectSamplesBatch12
{
    ...
    static void Main()
    {
        ...
        var oldTalk = new List<string>();
        var gossip = new List<string>();
        var talkOther = new List<string>();
        var tingen = new List<string>();
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        {
                            if (tags.Contains("oldtalk") && oldTalk.Count < 5) oldTalk.Add(enVal);
                            if (tags.Contains("gossip") && gossip.Count < 5) gossip.Add(enVal);
                            if (tags.Contains("talkother") && talkOther.Count < 5) talkOther.Add(enVal);
                            if ((tags.Contains("tingentalk") || tags.Contains("tingen")) && tingen.Count < 5) tingen.Add(enVal);
                        }
                    }
                }
            }
        ...
        Console.WriteLine("\n--- Sample OldTalk ---");
        foreach (var s in oldTalk) Console.WriteLine("  " + s);

        Console.WriteLine("\n--- Sample Gossip ---");
        foreach (var s in gossip) Console.WriteLine("  " + s);

        Console.WriteLine("\n--- Sample TalkOther ---");
        foreach (var s in talkOther) Console.WriteLine("  " + s);

        Console.WriteLine("\n--- Sample Tingen ---");
        foreach (var s in tingen) Console.WriteLine("  " + s);
    }
}
