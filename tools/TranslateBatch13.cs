...
class TranslateBatch13
{
    ...
    static readonly Dictionary<string, string> CanonExact = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // Титулы и предметы
        { "Dominator's Clash: King of Angels", "Битва Владык: Король Ангелов" },
        { "Dominator's Clash: Conquering Emperor", "Битва Владык: Покоряющий Император" },
        { "Dominator's Clash: Lord of Stars", "Битва Владык: Повелитель Звёзд" },
        { "Dominator's Clash: Bloody Prince", "Битва Владык: Кровавый Принц" },
        { "Light of the Double-Agent", "Свет предателя" },
        { "Light of the Double Agent", "Свет предателя" }
    };
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(string.Format("=== Запуск перевода Пакета №13 (Цель: {0} уникальных строк) ===", targetCount));
        ...
        Console.WriteLine(string.Format("Текущий размер словаря: {0} записей", existingRu.Count));

        // 2. Чтение индексов LSI
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        if (lsiTagMap.TryGetValue(sk, out tags))
                        {
                            ...
                            bool isItem = tags.Contains("itemnormal") || tags.Contains("item") || tags.Contains("itemgift") || tags.Contains("itemoutlook");
                            ...
                            {
                                seen.Add(cnKey); seen.Add(enVal);
                                pItemNormal.Add(Tuple.Create(cnKey, enVal));
                            }
                            else if (isTingen)
                            {
                                seen.Add(cnKey); seen.Add(enVal);
        ...
        Console.WriteLine(string.Format("Найдено кандидатов:\n  - OldTalk (карточные клубы, байки, воспоминания): {0}\n  - AsideTalk (уличные сценки, мысли, знаки): {1}\n  - OtherTalk (душевные беседы): {2}\n  - Item (предметы, описания, титулы, подарки): {3}\n  - Tingen/TingenTalk (городские сцены Тингена): {4}\n  - TalkOther (поручения и встречи): {5}\n  - Gossip (городские сплетни): {6}",
            pOldTalk.Count, pAsideTalk.Count, pOtherTalk.Count, pItemNormal.Count, pTingen.Count, pTalkOther.Count, pGossip.Count));

        // Формирование набора для перевода (3500 строк):
        ...
        // 2. AsideTalk (800 строк)
        int asideLimit = Math.Min(800, pAsideTalk.Count);
        for (int i = 0; i < asideLimit; i++) candidates.Add(pAsideTalk[i]);
        Console.WriteLine(string.Format("Добавлено AsideTalk: {0}", asideLimit));

        // 3. OtherTalk (600 строк)
        int otherLimit = Math.Min(600, pOtherTalk.Count);
        for (int i = 0; i < otherLimit; i++) candidates.Add(pOtherTalk[i]);
        Console.WriteLine(string.Format("Добавлено OtherTalk: {0}", otherLimit));

        // 4. ItemNormal / Item (600 строк)
        int itemLimit = Math.Min(600, pItemNormal.Count);
        for (int i = 0; i < itemLimit; i++) candidates.Add(pItemNormal[i]);
        Console.WriteLine(string.Format("Добавлено Item: {0}", itemLimit));

        // 5. Tingen (400 строк)
        int tingenLimit = Math.Min(400, pTingen.Count);
        for (int i = 0; i < tingenLimit; i++) candidates.Add(pTingen[i]);
        Console.WriteLine(string.Format("Добавлено Tingen: {0}", tingenLimit));

        // 6. TalkOther (100 строк)
        int talkOtherLimit = Math.Min(100, pTalkOther.Count);
        ...
        if (candidates.Count < targetCount)
        {
            ...
            for (int i = itemLimit; i < pItemNormal.Count && candidates.Count < targetCount; i++) candidates.Add(pItemNormal[i]);
        ...
        Console.WriteLine(string.Format("Итого отобрано в Пакет №13: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №13 успешно интегрирован!");
    }
