...
class TranslateBatch10CityDialogues
{
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("==========================================================================");
        Console.WriteLine(" LOTM - Пакет №10: Городские диалоги Тингена, сплетни и клубные встречи   ");
        Console.WriteLine("==========================================================================");
        ...
        Console.WriteLine(string.Format("Найдено кандидатов:\n  - Tingen/TingenTalk (городские диалоги Тингена): {0}\n  - Gossip (городские сплетни и слухи): {1}\n  - OldTalk (карточные клубы и тайные встречи): {2}\n  - OtherTalk (душевные беседы): {3}\n  - AsideTalk (уличные реплики): {4}",
            pTingen.Count, pGossip.Count, pOldTalk.Count, pOtherTalk.Count, pAsideTalk.Count));

        // Формирование набора для перевода (по 1000 из трех ключевых групп):
        var candidates = new List<Tuple<string, string>>();

        // 1. Tingen диалоги (1000 строк)
        int tingenLimit = Math.Min(1000, pTingen.Count);
        for (int i = 0; i < tingenLimit; i++)
        {
            candidates.Add(pTingen[i]);
        }
        Console.WriteLine(string.Format("Добавлено Tingen: {0}", tingenLimit));

        // 2. Gossip городские сплетни (1000 строк)
        int gossipLimit = Math.Min(1000, pGossip.Count);
        for (int i = 0; i < gossipLimit; i++)
        {
            candidates.Add(pGossip[i]);
        }
        Console.WriteLine(string.Format("Добавлено Gossip: {0}", gossipLimit));

        // 3. OldTalk клубные партии и встречи (1000 строк)
        ...
        // Если до 3000 не хватает, добираем из оставшихся групп
        if (candidates.Count < targetCount)
        {
            int before = candidates.Count;
            for (int i = tingenLimit; i < pTingen.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pTingen[i]);
            }
            for (int i = gossipLimit; i < pGossip.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pGossip[i]);
            }
            for (int i = oldLimit; i < pOldTalk.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pOldTalk[i]);
            }
            for (int i = 0; i < pOtherTalk.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pOtherTalk[i]);
            }
            for (int i = 0; i < pAsideTalk.Count && candidates.Count < targetCount; i++)
            {
        ...
        Console.WriteLine(string.Format("Итого отобрано в Пакет №10: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №10 успешно интегрирован!");
    }
