...
class TranslateBatch20
{
    ...
    static string CachePath = @"d:\gameDev\translate lotm\tools\batch20_cache.tsv";
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(string.Format("=== Запуск перевода Пакета №20 (Лимит: {0} уникальных строк) ===", maxTarget));
        ...
        // 3. Сбор кандидатов для Пакета №20
        ...
        Console.WriteLine(string.Format("Отобрано для Пакета №20: {0} уникальных строк (из {1} кандидатов)", candidates.Count, scoredList.Count));
        ...
        Console.WriteLine(string.Format("Пакет №20 успешно завершен! Новый размер словаря: {0} записей.", existingRu.Count));
    }
