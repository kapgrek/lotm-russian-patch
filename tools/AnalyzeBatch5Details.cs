...
class AnalyzeBatch5Details
{
    ...
    static void Main()
    {
        ...
        string[] targetTags = new string[] { "loading", "guide", "achievement", "newbietask", "traingame", "buffappear", "buffdata" };
        foreach (var tag in targetTags) categories[tag] = new List<Tuple<string, string>>();
                    ...
                    if (delim > 0)
                    {
                        ...
                        if (lsiTagMap.ContainsKey(sk))
                        {
                            foreach (var tag in lsiTagMap[sk])
                            {
                                ...
                                {
                                    categories[tag].Add(Tuple.Create(cnKey, enVal));
                                    break;
                                }
                            }
                        }
        ...
        {
            Console.WriteLine(string.Format("\n=== Tag: {0} (Total: {1}) ===", kvp.Key, kvp.Value.Count));
            for (int i = 0; i < Math.Min(5, kvp.Value.Count); i++)
            {
                Console.WriteLine(string.Format("  [{0}] EN: {1}", kvp.Key, kvp.Value[i].Item2));
            }
        }
    }
}
