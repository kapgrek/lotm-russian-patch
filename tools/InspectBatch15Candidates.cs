...
class InspectBatch15Candidates
{
    ...
    static void Main()
    {
        ...
        var itemCandidates = new List<Tuple<string, string>>();
        var dialogueCandidates = new List<Tuple<string, string>>();
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        if (lsiTagMap.TryGetValue(sk, out tags))
                        {
                            ...
                            else if (tags.Contains("item") || tags.Contains("itemnormal") || tags.Contains("itemgift") || tags.Contains("itemoutlook")) itemCandidates.Add(Tuple.Create(cnKey, enVal));
                            else if (tags.Contains("talk") || tags.Contains("talkother") || tags.Contains("gossip") || tags.Contains("oldtalk") || tags.Contains("asidetalk") || tags.Contains("othertalk") || tags.Contains("tingen")) dialogueCandidates.Add(Tuple.Create(cnKey, enVal));
        ...
        Console.WriteLine("\n=== Tagged Candidate Breakdown for Batch 15 ===");
        Console.WriteLine("  monsterskill: " + monsterSkill.Count);
        Console.WriteLine("  buffdata: " + buffData.Count);
        Console.WriteLine("  buffappear: " + buffAppear.Count);
        Console.WriteLine("  item remaining: " + itemCandidates.Count);
        Console.WriteLine("  dialogue remaining: " + dialogueCandidates.Count);
        ...
        for (int i = 0; i < Math.Min(35, sortedTags.Count); i++)
        {
