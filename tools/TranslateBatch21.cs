...
class TranslateBatch21
{
    ...
    static string CachePath = @"d:\gameDev\translate lotm\tools\batch21_cache.tsv";
    ...
    static int GetStringPriorityScore(string enVal, string cnKey)
    {
        ...
        // 6. Связный текст (содержит пробел)
        if (enVal.Contains(" "))
        {
            score += 20;
        }
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(string.Format("=== Запуск перевода Пакета №21 (Лимит: {0} уникальных строк) ===", maxTarget));
        ...
        // 3. Сбор кандидатов для Пакета №21
        ...
        Console.WriteLine(string.Format("Отобрано для Пакета №21: {0} уникальных строк (из {1} кандидатов)", candidates.Count, scoredList.Count));
        ...
        Console.WriteLine(string.Format("Пакет №21 успешно завершен! Новый размер словаря: {0} записей.", existingRu.Count));
    }
    ...
    static void SaveLuaDictionary(string filePath, List<string> orderedKeys, Dictionary<string, string> dict)
    {
        ...
        // Атомарное перемещение с повторными попытками
