...
class TranslateBatch9DialogueTaverns
{
    ...
    static void Main(string[] args)
    {
        ...
        int targetCount = 2700;
        if (args.Length > 0) int.TryParse(args[0], out targetCount);

        Console.WriteLine("==========================================================================");
        Console.WriteLine(" LOTM - Пакет №9: Диалоги жителей, таверн, сюжет и карточные клубы          ");
        Console.WriteLine("==========================================================================");
        ...
        Console.WriteLine(string.Format("Найдено кандидатов:\n  - MainTask (основной сюжет): {0}\n  - OtherTalk (душевные и тавернные диалоги): {1}\n  - AsideTalk (уличные реплики и мысли): {2}\n  - OldTalk (карточные партии и встречи): {3}",
            pMainTask.Count, pOtherTalk.Count, pAsideTalk.Count, pOldTalk.Count));

        // Формирование набора для перевода:
        // 1. Все оставшиеся MainTask (завершение сюжетной ветки на 100%)
        // 2. 700 OtherTalk
        // 3. 500 AsideTalk
        // 4. 500 OldTalk
        var candidates = new List<Tuple<string, string>>();

        // 1. Все оставшиеся сюжетные задания MainTask
        foreach (var c in pMainTask)
        {
            candidates.Add(c);
        }
        Console.WriteLine(string.Format("Добавлено MainTask (100% покрытие): {0}", candidates.Count));
        ...
        int asideAdded = 0;
        foreach (var c in pAsideTalk)
        {
            if (asideAdded >= 500) break;
            candidates.Add(c);
            asideAdded++;
        }
        Console.WriteLine(string.Format("Добавлено AsideTalk: {0}", candidates.Count - before));

        // 4. Карточные партии и реплики OldTalk
        before = candidates.Count;
        int oldAdded = 0;
        foreach (var c in pOldTalk)
        {
            if (oldAdded >= 500) break;
            candidates.Add(c);
            oldAdded++;
        }
        Console.WriteLine(string.Format("Добавлено OldTalk: {0}", candidates.Count - before));

        // 5. Если лимит еще не достигнут, добираем из оставшихся OtherTalk / AsideTalk
        if (candidates.Count < targetCount)
        {
            before = candidates.Count;
            for (int i = 700; i < pOtherTalk.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pOtherTalk[i]);
            }
            for (int i = 500; i < pAsideTalk.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pAsideTalk[i]);
            }
            for (int i = 500; i < pOldTalk.Count && candidates.Count < targetCount; i++)
            {
                candidates.Add(pOldTalk[i]);
            }
            Console.WriteLine(string.Format("Дополнительно добрано до лимита: {0}", candidates.Count - before));
        }

        Console.WriteLine(string.Format("Итого отобрано в Пакет №9: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine("Пакет №9 успешно интегрирован!");
    }
    ...
    static string ApplyCanonGlossary(string text)
    {
        ...
        // Специфика карточных партий и диалогов Пакета №9
