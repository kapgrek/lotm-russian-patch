...
class TranslateBatch23
{
    ...
    static string CachePath = @"d:\gameDev\translate lotm\tools\batch23_cache.tsv";
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(string.Format("=== Запуск перевода Пакета №23 (Лимит: {0} уникальных строк) ===", maxTarget));
        ...
        // 3. Сбор кандидатов для Пакета №23
        ...
        Console.WriteLine(string.Format("Отобрано для Пакета №23: {0} уникальных строк (из {1} кандидатов)", candidates.Count, scoredList.Count));
        ...
        Console.WriteLine(string.Format("Пакет №23 успешно завершен! Новый размер словаря: {0} записей.", existingRu.Count));
    }
