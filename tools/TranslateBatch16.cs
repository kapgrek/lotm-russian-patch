...
class TranslateBatch16
{
    ...
    static string CachePath = @"d:\gameDev\translate lotm\tools\batch16_cache.tsv";
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(string.Format("=== Запуск перевода Пакета №16 (Лимит: {0} уникальных строк) ===", maxTarget));
        ...
        // 3. Сбор кандидатов по приоритетным группам Пакета №16
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (t.StartsWith("[\"") && (t.EndsWith("\",") || t.EndsWith("\"")))
                {
                    ...
                    if (delim > 0)
                    {
                        ...
                        // Приоритет 1: 0-08, Honorific, The Fool, Grey Fog
                        ...
                        // Приоритет 2: Активности мира, Синтез спутников, пазлы, Тинген
                        ...
                        // Приоритет 3: Ремесла, мебель, зелья, предметы
                        ...
                        // Приоритет 4: RichText квесты, диалоги, достижения, TRPG
                        else if (enVal.StartsWith("<") && enVal.Contains("</>"))
                        {
                            listRichTextAndStory.Add(Tuple.Create(cnKey, enVal));
                        }
                        // Приоритет 5: Системные подсказки, промпты, уведомления
        ...
        candidates.AddRange(list008AndHonorific);
        candidates.AddRange(listWorldAndSynthesis);
        candidates.AddRange(listLifeAndItems);
        candidates.AddRange(listRichTextAndStory);
        candidates.AddRange(listSystemAndUI);
        ...
        Console.WriteLine(string.Format("Отобрано для Пакета №16: {0} уникальных строк", candidates.Count));
        ...
        Console.WriteLine(string.Format("Пакет №16 успешно завершен! Новый размер словаря: {0} записей.", existingRu.Count));
    }
